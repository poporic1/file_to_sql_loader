...
Простой локальный инструмент для загрузки CSV-файла в базу данных DuckDB, приведения данных к нормальному виду и формирования базового profiling-отчёта.
...
- в `sales_clean`:
  ...
  - `sale_date` имеет тип DATE
  - у одной строки `amount = NULL`
  - города нормализованы

В `profile_report.csv`:

- для `amount` `null_count = 1`
